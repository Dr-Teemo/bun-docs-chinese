---
title: 在 DigitalOcean 上部署 Bun 应用程序
sidebarTitle: 在 DigitalOcean 上部署
mode: center
---

[DigitalOcean](https://www.digitalocean.com/) 是一个云平台，为构建和部署应用程序提供一系列服务。
...
<Note>
    ...
    - 已安装并配置 [DigitalOcean CLI](https://docs.digitalocean.com/reference/doctl/how-to/install/#step-1-install-doctl)
    - 已安装 [Docker](https://docs.docker.com/get-started/get-docker/) 并添加到您的 `PATH`
...
<Steps>
	<Step title="创建新的 DigitalOcean 容器注册表">
		创建一个新的容器注册表来存储 Docker 镜像。

    	<Tabs>
    		<Tab title="通过 DigitalOcean 仪表板">
    			在 DigitalOcean 仪表板中，转到 [**容器注册表**](https://cloud.digitalocean.com/registry)，并输入新注册表的详细信息。
    	...
    	您应该在 [**DigitalOcean 注册表仪表板**](https://cloud.digitalocean.com/registry) 中看到新注册表：
    ...
    <Step title="创建新的 Dockerfile">
    	...
    	<Note>
    		确保启动命令对应于应用程序的入口点。如果您的 [package.json] 中有启动脚本，也可以是 `CMD ["bun", "run", "start"]`。

    		此镜像在容器内使用 Bun 安装依赖项并运行您的应用程序。如果您的应用程序没有依赖项，您可以省略 `RUN bun install --production --frozen-lockfile` 这一行。
    	</Note>
    ...
    <Step title="使用 DigitalOcean 注册表对 Docker 进行身份验证">
    ...
    <Step title="构建并将 Docker 镜像推送到 DigitalOcean 注册表">
    	...
    	<Note>
    		如果您在 ARM Mac (M1/M2) 上构建，则必须使用 `docker buildx` 和 `--platform=linux/amd64` 以确保与 DigitalOcean 基础设施的兼容性。使用 `docker build` 而不带平台标志将创建一个在 DigitalOcean 上无法运行的 ARM64 镜像。
    	</Note>

    	镜像推送后，您应该在 [**DigitalOcean 注册表仪表板**](https://cloud.digitalocean.com/registry) 中看到它：
    ...
    <Step title="创建新的 DigitalOcean App Platform 项目">
    	在 DigitalOcean 仪表板中，转到 [**App Platform**](https://cloud.digitalocean.com/apps) > **创建应用**。我们可以直接从容器镜像创建一个项目。
